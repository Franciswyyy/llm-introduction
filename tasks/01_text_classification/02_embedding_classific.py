...
PROJECT_ROOT = Path(__file__).parent.parent.parent  # 需要回到两级目录
...
def load_sentence_transformer():
    """
    加载预训练的Sentence Transformer模型 (使用配置化缓存系统)
    
    Returns:
        SentenceTransformer: 已加载的模型实例
    """
    # 使用统一的模型加载系统：自动缓存 + 设备选择 + 配置化
...
def generate_embeddings(model, data):
    ...
    # 将训练集文本转换为嵌入向量
    print("  正在处理训练集...")
    train_texts = list(data["train"]["text"])  # 转换为列表格式
    train_embeddings = model.encode(train_texts, show_progress_bar=True)
    
    # 将测试集文本转换为嵌入向量  
    print("  正在处理测试集...")
    test_texts = list(data["test"]["text"])   # 转换为列表格式
